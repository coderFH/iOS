...
简要说出点击黄色view,系统是如何找到最合适的控件来处理事件?
    UIApplication将触摸事件传递给了UIWindow, UIWindow -> 白色的view. 此时 whiteView 判断能接受触摸事件, 触摸点在自己身上,然后从后往前遍历子控件,先遍历到橙色的view,然后再去重复1,2步的检测,然后遍历到蓝色的view,蓝色view再重复1,2步的检测,会遍历到黄色的view,发现黄色的view都满足这些条件,并没有子控件,所以红色的view就是最佳的响应者
